...
diabetes = read.csv("/Users/dheerajreddy/Downloads/diabetes.csv")
diabetes
...
barplot(stats, main="Comparison of Glucose Statistics", ylab="Glucose", col=c("orange", "green", "orange", "green"), ylim=c(0, 250))
...
hist(bmi, breaks=20, xlab="BMI", main="BMI Distribution for Population",col="yellow",border = "black")
abline(v=data_98th_percentile, col="darkgreen", lty=2)
legend("topright", legend=c("98th percentile"), col="darkgreen", lty=2)

hist(sample(bmi, length(bmi), replace=TRUE), breaks=20, xlab="BMI", main="BMI Distribution for Sample", col="pink",border = "black")
abline(v=samp_98th_percentile, col="maroon", lty=2)
legend("topright", legend=c("98th percentile"), col="maroon", lty=2)
...
lines(percentile_bp, lty = 2, col = "red")
